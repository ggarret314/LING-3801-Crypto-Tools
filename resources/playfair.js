...
var Playfair = {
	...
	get manualMode() { return document.getElementById("playfair-manual-mode").checked },
	...
	/*
	proc find_key (seed_key)
		key = full_key(seed_key)
		pt = decipher(ct)
		bestFitness = Fitness(pt)
		minFitness = bestFitness
		minKey = key
		bestKey = key
		for temp = 20, 0, -0.2
			for count = 0, 50000, 1
				newKey = changeKey(key) // does some random translation to the key
				pt = decipher(ct)
				newFitness = Fitness(pt)
				dF = newFitness - bestFitness
				if dF <= 0
	...
	*/
	_autoSolve: function(startKeyPhrase, Temp = 20, Step = 0.2, Count = 50000) {
		var self = this;
		...
		var pt = self._autoDecipher(startKeyPhrase);
	...
	_autoSolveVisualizer: function(startKeyPhrase, Temp = 20, Step = 0.2, Count = 50000) {
		var self = this;
		var newAlphabet = Alphabet.substring(0, 9) + Alphabet.substring(10);
		startKeyPhrase = self._noRepeats(startKeyPhrase);
...
/*
	...
	_manualEncipher: function () {
		...
		for (var i = 0; i < pt.length; i += 2) {
			var letter1 = pt[i], letter2 = pt[i + 1];
			var letter1Index = this.grid.indexOf(letter1);
			var letter2Index = this.grid.indexOf(letter2);

			if (letter1Index % 5 == letter2Index % 5) {
				// same column
				letter1Index = (letter1Index + 5) % 25;
				letter2Index = (letter2Index + 5) % 25;
			} else if (Math.floor(letter1Index / 5) == Math.floor(letter2Index / 5)) {
				// same row
				letter1Index = (letter1Index + 1) % 5 + 5 * Math.floor(letter1Index / 5);
				letter2Index = (letter2Index + 1) % 5 + 5 * Math.floor(letter2Index / 5);
			} else {
				// box rule
				var diff = letter2Index % 5 - letter1Index % 5;
				letter1Index += diff;
			...
			ct += this.grid[letter1Index] + this.grid[letter2Index];
			ct += " ";
		}
	...
	_manualDecipher: function () {
		...
		var decipheringContainer = document.getElementById("playfair-deciphering-container");
		decipheringContainer.innerHTML = "";

		var ct = this.manualCiphertextElement.value;
		
		ct = text_sanitize(ct);
		
		// error check the cipher text before continuing
		...
		for (var i = 0; i < ct.length; i += 2) {
			...
			cipherRow.innerHTML = ct.substring(i, i + 2);
			...
			if (letter1Index !== -1 && letter2Index !== -1) {
				...
				letter1 = this.grid[letter1Index]; letter2 = this.grid[letter2Index];
				letter1 = letter1 == "" ? "." : letter1;
				letter2 = letter2 == "" ? "." : letter2;
				pt += letter1 + letter2;
				plainRow.innerHTML = letter1 + letter2;
			} else {
				pt += "..";
				plainRow.innerHTML = "..";
			}
			...
			decipheringContainer.appendChild(decipheringBlock);
		}
